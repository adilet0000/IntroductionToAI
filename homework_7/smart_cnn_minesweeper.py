...
from minesweeper import MinesweeperEnv  # Своё улучшенное окружение

# --- CNN агент ---
class SmartCNNAgent(nn.Module):
   def __init__(self, width=30, height=16):
      super(SmartCNNAgent, self).__init__()
      self.width = width
      self.height = height
      self.conv1 = nn.Conv2d(3, 32, kernel_size=3, padding=1)  # 3 канала: открытость, число, флажок
...
class MinesweeperDQN:
   def __init__(self, env, gamma=0.99, epsilon=1.0, epsilon_min=0.05, epsilon_decay=0.995, batch_size=128, memory_size=100000):
      ...
      self.model = SmartCNNAgent(width=self.width, height=self.height).to(self.device)
   ...
   def save(self, path="smart_cnn_minesweeper.pth"):
...
def play_one_episode(dummy):
   ...
   state = env._get_processed_observation()
   done = False
   trajectory = []
   ...
   while not done:
      ...
      next_state = env._get_processed_observation()
...
def train_parallel(n_processes=4, n_episodes=10000, checkpoint_interval=1000):
   ...
   # --- Авто-подгрузка чекпоинтов ---
   checkpoints = sorted(glob.glob("smart_cnn_minesweeper_checkpoint_*.pth"))
   start_episode = 0
   if checkpoints:
   ...
   for episode_batch in range(start_episode, n_episodes, n_processes):
      ...
      if episode_batch > 0 and episode_batch % checkpoint_interval == 0:
         checkpoint_path = f"smart_cnn_minesweeper_checkpoint_{episode_batch}.pth"
         agent.save(checkpoint_path)
         print(f"Checkpoint saved to {checkpoint_path}")

   agent.save("smart_cnn_minesweeper.pth")
   print("Final model saved to smart_cnn_minesweeper.pth")
...
def test():
   env = MinesweeperEnv(width=30, height=16, n_mines=99)
   agent = MinesweeperDQN(env)
   agent.load("smart_cnn_minesweeper.pth")
